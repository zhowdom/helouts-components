<template>
  <div class="eachCardBox clearfix">
    <div class="energyStatistic bdbox commonTitlebox comBoderAndBg pureSetBG" style="width:520px;height:222px;">
                ...
                <div id="echrMain3Bar" style="width: 495px;height: 167px;"></div>
            </div>
  </div>
</template>
    ...
    myEcharts3Bar () {
      ...
      var option = {
        ...
        dataset: {
          ...
          source: this.cardData.map(v => {
            let {items} = v
            let s = {}
            for (let inner of Object.values(items)) {
              s['time'] = this.dayjs(v.date).format('MM-DD')
